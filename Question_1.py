...
for i,a in enumerate(influenza_genome):		#adding counter (i) to the list (a) using enumerate
	
	if i == 300:		#checking the counter if its at 300th postition
		one_a = a+1	#adding 1 to the 300th position 
		print(i, end = " ")


	if i <=31:			#counter should be less than 30
		print(a, end = " ")
		if a % 3 == 0:		#checking if elements in list are divisible by 3 ( % will give us reminder, if reminder is 0, then we will know that it is divisible by 3) 
			one_b = a*3	#multiplying the values that are divisible by 3 with 3 
		

	if i >= 470:			#selecting the last 30 elements in the list 
		if i % 5 == 0:		#checking if counter values are multiple of 5
			 t = str(a)	#converting list (a) to string 
			 one_c = t.replace(t,'a')	#replacing values that are divisible by 5 with a 


	if 200 < i < 301:		#setting parameter so that counter value is in between 200 and 300
		if a % 3 == 0 and a % 5 == 0:	#checking if value in list is multiple of 3 and 5 
			print(a) 
			convert = str(a)	# converting values in list to string 
			one_d = convert.replace(convert,'10')	#replacing values that are divisible by 3 and 5 with 10
